...
public class Forum {
    ...
    private static void forum(SLL<Discussion> health, SLL<Discussion> finance) {
        ...
        SLLNode<Discussion> discussion = finance.getFirst();

        while (tmp != null) {
            if (tmp.element.calculateImportance() < discussion.element.calculateImportance())
                discussion = tmp;
            tmp = tmp.succ;
        }
        finance.delete(discussion);

        tmp = health.getFirst();
        discussion = health.getFirst();
        ...
        finance.insertLast(new Discussion(discussion.element.getId(), discussion.element.getPopularity(), discussion.element.getUsers()));
    }
